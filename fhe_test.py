...
def nearest_power_padding(str_obj, obj_len):
    ...
    if (2**exp == obj_len):
        return str_obj # if the length is already a power of two, return the string as is
...
class OpenFHEScheme(FHEScheme):
    def __init__(self, seed):
        self.seed=seed # the seed value will be used for key generation, packing plaintext, encryption, and decryption
    def getKeys(self):
    ...
    def encrypt(self, key, plaintext):
        ptx=self.seed.MakeCKKSPackedPlaintext(plaintext) # converts the list of values to a plaintext object
        cipher=self.seed.Encrypt(key, ptx)
        return cipher
    def decrypt(self, key, ciphertext):
...
def test_enc_dec_nums_ofhe(model, plaintext):
    ...
    ofhe_scheme=OpenFHEScheme(cc) # create OpenFHEScheme object
    private, public = ofhe_scheme.getKeys() # generates two keys to be stored in the "keys" variable

    c1 = ofhe_scheme.encrypt(public, plain_t) # encrypt the plaintext object using the public key
    dec = ofhe_scheme.decrypt(private, c1) # decrypt using the secret key
    assert np.all(plain_t == dec) #np.all checks if array elements are the same
